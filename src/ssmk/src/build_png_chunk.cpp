...
void ssmk::build_png_chunk() {
	...
	*(std::uint32_t*)m_context.im.chunk = htonl(n);
	for (std::vector<std::string>::size_type i = 0; i < paths.size(); i++) {
		...
		std::memcpy(m_context.im.chunk + at, &x, sizeof(std::uint32_t)); // x
		at += sizeof(std::uint32_t);
		std::memcpy(m_context.im.chunk + at, &y, sizeof(std::uint32_t)); // y
		at += sizeof(std::uint32_t);
		std::memcpy(m_context.im.chunk + at, &w, sizeof(std::uint32_t)); // w
		at += sizeof(std::uint32_t);
		std::memcpy(m_context.im.chunk + at, &h, sizeof(std::uint32_t)); // h
		at += sizeof(std::uint32_t);

		slen = paths[i].length() + 1;

		std::memcpy(m_context.im.chunk + at, paths[i].c_str(), slen);
		at += slen;
	...
	m_context.im.png_chunk = new png_unknown_chunk;
	std::memcpy(m_context.im.png_chunk->name, chunk_name, 5);
	m_context.im.png_chunk->data = (png_bytep)m_context.im.chunk;
	m_context.im.png_chunk->size = m_context.im.chunk_size;
	m_context.im.png_chunk->location = PNG_HAVE_PLTE; // write chunk before IDAT
	png_set_unknown_chunks(
		(png_structp)m_context.im.png, 
		(png_infop)m_context.im.info, 
		m_context.im.png_chunk, 1
	);
